...
smoketest:
    @just run-avoidance --n-random-traces=1 --timeout=20
...
# You can add `-j N` to run on `N` cores. For example,
# the command `@just run-avoidance --n-random-traces=10 -j 4`
# will run experiments on 4 cores
run-short:
    @just run-avoidance --n-random-traces=10 --timeout=60
    @just run-f16  --n-random-traces=5 --timeout=120
    @echo "------------------------"
    @just results

run-medium:
    @just run-avoidance --n-random-traces=50 --timeout=60
    @just run-f16 --n-random-traces=10 --timeout=120
    @echo "------------------------"
    @just results
